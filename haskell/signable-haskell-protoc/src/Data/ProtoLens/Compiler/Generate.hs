...
import HsSyn (ideclAs, ideclName)
import Lens.Family2 ((^.))
...
generateModule ::
  ModuleNameStr ->
  -- | The complete file descriptor
  FileDescriptorProto ->
  -- | The imported modules
  [ModuleNameStr] ->
  -- | The publicly imported modules
  [ModuleNameStr] ->
...
generateModule modName fdesc imports publicImports definitions importedEnv services =
  ...
  where
    ...
    pragmas =
      [ languagePragma $ List.intercalate ", " $
          map
            fromString
            [ "ScopedTypeVariables",
              "DataKinds",
  ...
  where
    ...
    item l =
      Outputable.text
        ...
        Outputable.<> (Outputable.ppr $ var "Lens'" @@ t @@ (lensFieldType l))
...
generateServiceDecls env si =
  ...
  [ data'
    ...
    [ instance'
        (var "Data.ProtoLens.Service.Types.Service" @@ serverRecordType)
    ...
    [ instance'
        (var "Data.ProtoLens.Service.Types.HasMethodImpl" @@ serverRecordType @@ instanceHead)
...
generateMessageDecls fieldModName env protoName info =
  ...
  [ commented (messageComment fieldModName (messageName info) allFields) $
    ...
    [ uncommented $ instance' (var "Data.ProtoLens.Message" @@ dataType) $
      ...
      uncommented $
        instance'
          (var "Control.DeepSeq.NFData" @@ dataType)
...
generateEnumDecls info =
  ...
  [ newtype'
    ...
    [ data'
      ...
      instance'
        (var "Data.ProtoLens.MessageEnum" @@ dataType)
      ...
      instance'
        (var "Data.ProtoLens.FieldDefault" @@ dataType)
  ...
  where
    ...
    errorMessage =
      "toEnum: unknown value for enum " ++ unpack (ed ^. #name)
        ++ ": "
    errorMessageExpr =
      var "Prelude.error"
    ...
    succDecl funName boundName thePairs =
      funBinds funName $
        match
          [conP_ (unqual boundName)]
          ( var "Prelude.error"
              @@ string
                ( concat
                    [ occNameStrToString dataName,
                      ".",
                      occNameStrToString funName,
                      ": bad argument ",
          ...
          ++ [ match
                 [conP (unqual $ unrecognizedName u) [wildP]]
                 ( var "Prelude.error"
                     @@ string
                       ( concat
                           [ occNameStrToString dataName,
                             ".",
                             occNameStrToString funName,
                             ": bad argument: unrecognized value"
                           ]
                       )
                 )
               | Just u <- [unrecognized]
...
data RecordField
  = RecordField
      { -- | The Haskell name of this field (unique
        --   within the module).
        recordFieldName :: OccNameStr,
        -- | Internal type in the record
        recordFieldType :: HsType',
...
hsFieldValueDefault env fd = case fd ^. #type' of
  ...
  FieldDescriptorProto'TYPE_ENUM
    ...
    | Enum e <- definedFieldType fd env,
      Just v <-
        List.lookup
          def
          [ (enumValueDescriptor v ^. #name, enumValueName v)
            | v <- enumValues e
          ] ->
      var v
  ...
  where
    ...
    errorMessage fieldType =
      error $
        "Bad default value " ++ show (T.unpack def)
          ++ " in default value for "
          ++ fieldType
          ++ " field "
          ++ unpack (fd ^. #name)
...
messageInstance env protoName m =
  ...
  where
    ...
    fieldDescriptorName f =
      nameFromSymbol $
        overloadedName (fieldName . plainFieldInfo $ f)
          <> "__field_descriptor"
...
textFormatFieldName env descr = case descr ^. #type' of
  FieldDescriptorProto'TYPE_GROUP
    ...
    | otherwise ->
      error $
        "expected TYPE_GROUP for type name"
...
fieldTypeDescriptorExpr = \case
  ...
  where
    mk x y =
      var (fromString ("Data.ProtoLens." ++ x))
  ...
  where
    fieldNames =
      messageUnknownFields msg
        : map
          (haskellRecordFieldName . fieldName . plainFieldInfo)
          (messageFields msg)
        ++ map
          (haskellRecordFieldName . oneofFieldName)
...
oneofRnfMatch c =
  match [unqual (caseConstructorName c) `conP` [bvar "x__"]] $
