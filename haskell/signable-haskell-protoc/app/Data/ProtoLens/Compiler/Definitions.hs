...
data FieldPacking
  = -- | Cannot be packed (e.g., strings or messages).
    NotPackable
  | -- | Can be decoded as packed, but should not be not be
    --   encoded as packed by default.
    Packable
  | -- | Can be decoded as packed, and should be encoded
...
definedFieldType fd env = fromMaybe err $ Map.lookup (fd ^. #typeName) env
  where
    err =
      error $
        "definedFieldType: Field type " ++ unpack (fd ^. #typeName)
...
collectOneofFields ::
  String ->
  DescriptorProto ->
  Map.Map (Maybe Int32) [FieldDescriptorProto] ->
  [OneofInfo]
...
camelCase s =
  ...
  let (underlines, rest) = T.span (== '_') s
   in case splitOn "_" rest of
        ...
        [] ->
          error $
            "camelCase: splitOn returned empty list: "
              ++ show rest
        [""] ->
          error $
            "camelCase: name consists only of underscores: "
              ++ show s
...
enumDef syntaxType protoPrefix hsPrefix d =
  let mkText n = protoPrefix <> n
      mkHsName n = fromString $
        hsPrefix ++ case hsName n of
          ('_' : xs) -> 'X' : xs
          xs -> xs
   in ( mkText (d ^. #name),
        Enum
