  ...
  let fd = fieldDescriptor f
   in var $ case fd ^. #type' of
        ...
        FieldDescriptorProto'TYPE_MESSAGE
          ...
          | otherwise ->
            error $
              "expected TYPE_MESSAGE for type name"
                ++ unpack (fd ^. #typeName)
        ...
        FieldDescriptorProto'TYPE_ENUM
          ...
          | otherwise ->
            error $
              "expected TYPE_ENUM for type name"
                ++ unpack (fd ^. #typeName)
...
data FieldEncoding
  = FieldEncoding
      { -- | :: a -> Builder
        buildFieldType :: HsExpr',
        -- | :: Parser a
        parseFieldType :: HsExpr',
        wireType :: Word8
      }
  ...
  where
    ...
    parseLengthy =
      do'
        [ len <-- getVarInt',
          stmt $
            var "Data.ProtoLens.Encoding.Bytes.getBytes"
              @@ (fromIntegral' @@ len)
        ]
...
stringField = partialField (var "Data.Text.Encoding.encodeUtf8") decodeUtf8P lengthy
  where
    ...
    decodeUtf8P bytes =
      case'
        (var "Data.Text.Encoding.decodeUtf8'" @@ bytes)
...
isolatedLengthy parser =
  do'
    [ len <-- getVarInt',
      stmt $
        var "Data.ProtoLens.Encoding.Bytes.isolate"
          @@ (fromIntegral' @@ len)
          @@ parser
    ]
  where
