...
module Proto.Text (Payload()) where
...
{- | Fields :
 ...
 -}
data Payload = Payload{_Payload'text :: !Data.Text.Text,
...
instance Data.ProtoLens.Field.HasField Payload "text"
           (Data.Text.Text)
         where
        fieldOf _
          = (Lens.Family2.Unchecked.lens _Payload'text
               (\ x__ y__ -> x__{_Payload'text = y__}))
              Prelude.. Prelude.id
instance Data.ProtoLens.Message Payload where
        ...
        defMessage
          = Payload{_Payload'text = Data.ProtoLens.fieldDefault,
                         ...
                         do tag <- Data.ProtoLens.Encoding.Bytes.getVarInt
                            case tag of
                                10 -> do y <- (do value <- do len <- Data.ProtoLens.Encoding.Bytes.getVarInt
                                                ...
                                                Data.ProtoLens.Encoding.Bytes.<?> "text"
                                         loop
                                           (Lens.Family2.set (Data.ProtoLens.Field.field @"text") y
                                              x)
